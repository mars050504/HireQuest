...
export const register = async (req, res) => {
    try {
        ...
        return res.status(201).json({
            message: "Account created seccessfully.",
            success: true
        });
    } catch (error) {
        console.log(error);
...
export const login = async (req, res) => {
    try {
        ...
        if (role !== user.role) {
            return res.status(400).json({
                message: "Account doens't exist with current role.",
                success: false
            })
        };
        ...
        return res.status(200).cookie("token", token, { maxAge: 1 * 24 * 60 * 60 * 60 * 1000, httpsOnly: true, sameSite: 'strict' }).json({
